...
- **Palette:** Warm off-white background (`#FFFCF8`) + white cards + deep charcoal text (`#171717`), with a single warm-orange brand accent (`#f97316`) reserved for CTAs, active nav states and focus rings — it is never a background fill. Finance meaning is carried by three fixed hues that are never reused for anything else: emerald = income/approved, red = expense/rejected, amber = offering/pending. Max two background colors on any screen (white + off-white).
- **Type:** Sarabun for Thai UI text, Inter for Latin/UI/numerals (Inter renders Thai poorly, so the pairing is deliberate, not accidental). Body is 15px/1.6, headings are 600-weight with −0.02em tracking and `text-wrap: balance`. All money/amount text uses `.num-display` (tabular-nums, lining-nums) so digits align in columns.
...
- **Radius:** Buttons 18px, inputs 14px, cards 24px, dialogs 28px, sheets/drawers 32px, badges/pills fully round — but **tables and table cells are always 0px radius**; borders carry the depth cue there instead of rounding.
- **Cards:** White, 1px border (`var(--border)`), 24px radius, soft `shadow-sm-card`/`shadow-card` only — never a heavy shadow. Interactive cards get a 1px border color shift + 1px lift on hover, nothing flashier.
- **Elevation:** Border beats shadow as the default depth cue; real shadow is reserved for genuine overlap (dialogs, popovers) and one "lifted" hero surface per screen at most (see `surface-elevated`, used once on the dashboard's balance card).
