...
serve(async (req) => {
  ...
  try {
    ...
    const { data: userData, error: userError } = await supabase
      .from("users")
      .select(
        "id, auth_user_id, church_id, name, role, avatar_color, is_active, created_at, updated_at",
      )
      .eq("auth_user_id", user.id)
      .single();

    if (userError || !userData) {
      return new Response(JSON.stringify({ error: "User not found in church" }), {
        status: 404,
        headers: { ...corsHeaders(req), "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ user: userData, church: churchData }), {
      status: 200,
      headers: { ...corsHeaders(req), "Content-Type": "application/json" },
    });
  } catch (error) {
