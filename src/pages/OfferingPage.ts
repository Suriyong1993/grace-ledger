...
export class OfferingPage {
    ...
    if (this.mode === "list") {
      ...
      if (!this.selectedSession) {
        ...
        const nextStepHintHtml = needsCounting
          ? `<div class="gl-notice gl-notice--info" role="status" style="margin-bottom: var(--space-4);">
  ...
  private async handleSaveVarianceExplanation(
    onStateChange: () => void,
  ): Promise<void> {
    if (!this.selectedSession) return;
  ...
  private async handleVarianceRecount(
    onStateChange: () => void,
  ): Promise<void> {
    if (!this.selectedSession) return;
