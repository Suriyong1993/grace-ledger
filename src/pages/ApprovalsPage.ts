...
export class ApprovalsPage {
   ...
   * re-render) so it stays fast for a high-volume approval queue. The first
   * click turns the button into "ยืนยันอนุมัติ" with a sibling "ยกเลิก" to back
   * out; the second click on the same button actually approves.
   *
   * Money is only moved on the second tap: the first one changes nothing but
   * the label.
   */
