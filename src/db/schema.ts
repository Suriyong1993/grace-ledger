...
export const users = pgTable(
  "users",
  {
    ...
    authUserId: uuid("auth_user_id").unique(), // Supabase Auth FK reference
    name: varchar("name", { length: 255 }).notNull(),
    email: varchar("email", { length: 255 }), // For PIN-based login via Supabase Auth
  ...
  (table) => [
    uniqueIndex("idx_users_church_name")
...
export const fiscalPeriods = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_fp_church_year_period").on(
      table.churchId,
      table.fiscalYear,
      table.periodNumber,
    ),
...
export const journalEntries = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_journal_entries_number")
      .on(table.entryNumber)
      .where(sql`${table.entryNumber} IS NOT NULL`),
    ...
    check(
      "chk_no_self_approve_1",
      sql`${table.approval1Id} IS NULL OR ${table.approval1Id} <> ${table.createdBy}`,
    ),
    // CF-2: Two approvers must be different
    check(
      "chk_no_same_approvers",
      sql`${table.approval2Id} IS NULL OR ${table.approval1Id} IS NULL OR ${table.approval1Id} <> ${table.approval2Id}`,
    ),
    ...
    check(
      "chk_approved_has_approver",
      sql`${table.status} <> 'approved' OR ${table.approval1Id} IS NOT NULL`,
    ),
  ],
);
...
export const offeringCountSheets = pgTable(
  "offering_count_sheets",
  {
    ...
    counter1Id: uuid("counter_1_id")
      .notNull()
      .references(() => users.id),
    counter1Amount: decimal("counter_1_amount", { precision: 18, scale: 2 })
      .notNull()
      .default("0.00"),
    counter2Id: uuid("counter_2_id")
      .notNull()
      .references(() => users.id),
    counter2Amount: decimal("counter_2_amount", { precision: 18, scale: 2 })
      .notNull()
      .default("0.00"),
  ...
  (table) => [
    ...
    check(
      "chk_counters_different_13",
      sql`${table.counter1Id} <> ${table.counter3Id} OR ${table.counter3Id} IS NULL`,
    ),
    check(
      "chk_counters_different_23",
      sql`${table.counter2Id} <> ${table.counter3Id} OR ${table.counter3Id} IS NULL`,
    ),
    check(
      "chk_counter_amounts_positive",
      sql`${table.counter1Amount} >= 0 AND ${table.counter2Amount} >= 0`,
    ),
  ],
);
...
export const budgets = pgTable(
  "budgets",
  {
    ...
    departmentId: uuid("department_id"),
    projectId: uuid("project_id"),
    budgetedAmount: decimal("budgeted_amount", { precision: 18, scale: 2 })
      .notNull()
      .default("0.00"),
...
// ============================================================================
// Entry Number Counters (RACE CONDITION FIX: atomic entry number assignment)
// ============================================================================

export const entryNumberCounters = pgTable(
  ...
  (table) => [
    uniqueIndex("uq_counter_church_type_year").on(
      table.churchId,
      table.entryType,
      table.fiscalYear,
    ),
...
export const lineUsers = pgTable(
  ...
  (table) => [
    index("idx_line_users_line_id").on(table.lineUserId),
    index("idx_line_users_church").on(table.churchId),
  ],
);
