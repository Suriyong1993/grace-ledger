...
import { createExpense, deleteExpense, listCategories, listExpense, listFunds, setExpenseStatus } from "@/services/church";
...
function ExpensePage() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { toast.success("ลบแล้ว"); qc.invalidateQueries({ queryKey: ["expense"] }); },
  });
  const setStatus = useMutation({
    mutationFn: ({ id, s }: { id: string; s: "approved" | "rejected" }) => setExpenseStatus(id, s, user!),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["expense"] }); },
  });

  const form = useForm<Values>({
    resolver: zodResolver(schema),
    defaultValues: { date: today(), amount: 0, categoryId: "", fundId: "", vendor: "", description: "" },
  });
  ...
  const rows = (expQ.data ?? []).filter((r) => {
    ...
    return [r.description, r.vendor, catName(r.categoryId), fundName(r.fundId), String(r.amount)].some((v) => v?.toLowerCase().includes(s));
  });
  const total = rows.reduce((s, r) => s + r.amount, 0);

  const exportCsv = () => {
    const csv = toCsv(
      rows.map((r) => ({ date: r.date, category: catName(r.categoryId), fund: fundName(r.fundId), vendor: r.vendor ?? "", amount: r.amount, status: r.status, description: r.description ?? "" })),
      { date: "วันที่", category: "หมวดหมู่", fund: "กองทุน", vendor: "ผู้ขาย", amount: "จำนวน", status: "สถานะ", description: "รายละเอียด" },
    );
          ...
          <>
            ...
            {can("expense.write") && (
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild><Button className="rounded-2xl"><Plus className="h-4 w-4 mr-2" /> เพิ่มรายจ่าย</Button></DialogTrigger>
                    ...
                    <form onSubmit={form.handleSubmit((v) => create.mutate(v))} className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <FormField name="amount" control={form.control} render={({ field }) => (
                          <FormItem><FormLabel>จำนวนเงิน (บาท)</FormLabel><FormControl><Input type="number" step="0.01" className="rounded-xl" {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                      </div>
                      ...
                      <FormField name="vendor" control={form.control} render={({ field }) => (
                        <FormItem><FormLabel>ผู้ขาย / ผู้รับเงิน</FormLabel><FormControl><Input className="rounded-xl" {...field} /></FormControl></FormItem>
                      )} />
      ...
      <Card className="rounded-3xl overflow-hidden">
        <CardContent className="p-0">
          {expQ.isLoading ? (
            ...
            <div className="p-6"><EmptyState icon={ArrowUpCircle} title="ยังไม่มีรายจ่าย" description="เริ่มบันทึกรายจ่ายแรก" /></div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-right space-x-1">
                      {can("expense.approve") && r.status === "pending" && (
                        <>
                          <Button size="sm" variant="ghost" onClick={() => setStatus.mutate({ id: r.id, s: "approved" })} aria-label="อนุมัติ"><CheckCircle2 className="h-4 w-4 text-success" /></Button>
                          <Button size="sm" variant="ghost" onClick={() => setStatus.mutate({ id: r.id, s: "rejected" })} aria-label="ปฏิเสธ"><XCircle className="h-4 w-4 text-destructive" /></Button>
                        </>
                      )}
                      {can("expense.write") && <Button size="sm" variant="ghost" onClick={() => remove.mutate(r.id)} aria-label="ลบ"><Trash2 className="h-4 w-4 text-destructive" /></Button>}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
