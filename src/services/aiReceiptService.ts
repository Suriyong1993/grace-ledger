...
export function autoCategorize(text: string, merchant: string = ""): string {
  ...
  if (/น้ำมัน|ptt|or|bcp|shell|caltex|ทางด่วน|grab|แท็กซี่|ค่าเดินทาง/.test(query)) return "เดินทาง";
  if (/กระดาษ|ปากกา|สำนักงาน|officemate|ตลับหมึก|เครื่องเขียน|ปริ้นท์/.test(query)) return "สำนักงาน";
  if (/ค่าไฟ|ค่าน้ำ|การไฟฟ้า|การประปา|ais|true|dtac|อินเทอร์เน็ต|โทรศัพท์/.test(query)) return "สาธารณูปโภค";
  if (/โฆษณา|facebook|google|ads|การตลาด|โปรโมท|tiktok/.test(query)) return "การตลาด";
  if (/ข้าว|อาหาร|กาแฟ|amazon|starbucks|7-eleven|โลตัส|บิ๊กซี|ร้านอาหาร/.test(query)) return "อาหาร";
...
// AI Multi-Document Parser using Gemini 2.0 Flash Vision
export async function parseDocumentWithAI(file: File): Promise<ParsedDocumentResult> {
  ...
  if (AI_API_KEY) {
    try {
      const systemPrompt = `You are an expert AI Document OCR Parser for Thai Receipts, Tax Invoices (ใบกำกับภาษี), Bank Slips (สลิปโอนเงิน), Handwritten Bills (บิลเขียนมือ), and E-commerce Screenshots (Shopee/Lazada/TikTok).

Analyze the document image and extract fields in strict JSON format ONLY:
{
  "docType": "tax_invoice|receipt|bank_slip|handwritten|ecommerce_screenshot",
  ...
  "items": [{"name":"string", "price":number, "qty":number}]
      ...
      if (res.ok) {
        ...
        const parsed = JSON.parse(cleanJson) as ParsedDocumentResult;

        if (parsed.taxId) {
          parsed.isTaxIdValid = validateThaiTaxId(parsed.taxId);
        }

        if (!parsed.category) {
          parsed.category = autoCategorize(parsed.description, parsed.merchantName);
        }

        return parsed;
      }
    } catch (err) {
      console.warn("AI Vision Parsing fallback to simulated OCR:", err);
    }
  }

  // Fallback Rule-Based Parser
  const todayDate = new Date().toISOString().split("T")[0];
  const isShopeeLazada = file.name.toLowerCase().includes("shopee") || file.name.toLowerCase().includes("lazada");
  const isSlip = file.name.toLowerCase().includes("slip") || file.name.toLowerCase().includes("โอน");
