...
export function FundsGrid({ funds = DEFAULT_FUNDS }: FundsGridProps) {
  // NOTE: never substitute demo funds when the real list is genuinely empty —
  // see RecentTransactionsTable for the same fix and rationale.
  ...
  return (
    <div className="space-y-3">
      ...
      {displayFunds.length === 0 ? (
        <div className="card-ledger flex flex-col items-center justify-center gap-2 py-10 text-center">
          ...
          <p className="text-sm font-medium text-foreground">ยังไม่มีกองทุน</p>
          <p className="text-xs text-muted-foreground">เพิ่มกองทุนแรกเพื่อเริ่มติดตามยอดคงเหลือ</p>
        </div>
      ) : (
