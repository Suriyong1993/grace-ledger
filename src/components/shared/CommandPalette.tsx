...
import {
  ArrowDownCircle, ArrowUpCircle, HandHeart, Wallet, PiggyBank, Briefcase,
  Users, ScrollText, LayoutDashboard, FileBarChart2, Settings,
} from "lucide-react";
import {
  CommandDialog, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList, CommandSeparator,
} from "@/components/ui/command";
import {
  listAudit, listBudget, listExpense, listFunds, listIncome, listMembers, listOffering, listProjects, listCategories,
...
export function CommandPalette({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const results = useMemo(() => {
    const hit = (str: unknown) => !s || String(str ?? "").toLowerCase().includes(s);
    return {
      income: (inQ.data ?? []).filter((r) => hit(r.description) || hit(catName(r.categoryId)) || hit(fundName(r.fundId)) || hit(r.amount)).slice(0, 6),
      expense: (exQ.data ?? []).filter((r) => hit(r.description) || hit(r.vendor) || hit(catName(r.categoryId)) || hit(fundName(r.fundId)) || hit(r.amount)).slice(0, 6),
      offering: (offQ.data ?? []).filter((r) => hit(OFFERING_LABEL[r.type]) || hit(fundName(r.fundId)) || hit(r.amount)).slice(0, 6),
      ...
      members: (memQ.data ?? []).filter((r) => hit(r.name) || hit(r.family) || hit(r.department) || hit(r.phone) || hit(r.email)).slice(0, 6),
      audit: (audQ.data ?? []).filter((r) => hit(r.userName) || hit(r.action) || hit(r.entity) || hit(r.details)).slice(0, 6),
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [s, inQ.data, exQ.data, offQ.data, fundQ.data, budQ.data, projQ.data, memQ.data, audQ.data, catQ.data]);

  const go = (to: string) => { recent.push(q); onOpenChange(false); nav({ to }); };

  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      <CommandInput value={q} onValueChange={setQ} placeholder="ค้นหารายรับ, รายจ่าย, สมาชิก, กองทุน, โครงการ..." />
      <CommandList>
        ...
        {!s && (
          <>
            <CommandGroup heading="ทางลัด">
              <CommandItem onSelect={() => go("/dashboard")}><LayoutDashboard className="h-4 w-4 mr-2 text-primary" /> แดชบอร์ด</CommandItem>
              <CommandItem onSelect={() => go("/income")}><ArrowDownCircle className="h-4 w-4 mr-2 text-success" /> รายรับ</CommandItem>
              <CommandItem onSelect={() => go("/expense")}><ArrowUpCircle className="h-4 w-4 mr-2 text-destructive" /> รายจ่าย</CommandItem>
              <CommandItem onSelect={() => go("/offering")}><HandHeart className="h-4 w-4 mr-2 text-primary" /> เงินถวาย</CommandItem>
              <CommandItem onSelect={() => go("/funds")}><Wallet className="h-4 w-4 mr-2 text-secondary" /> กองทุน</CommandItem>
              <CommandItem onSelect={() => go("/reports")}><FileBarChart2 className="h-4 w-4 mr-2" /> รายงาน</CommandItem>
              <CommandItem onSelect={() => go("/settings")}><Settings className="h-4 w-4 mr-2" /> ตั้งค่า</CommandItem>
            </CommandGroup>
          ...
          <CommandGroup heading="รายรับ">
            {results.income.map((r) => (
              <CommandItem key={r.id} onSelect={() => go("/income")}>
                ...
                <span className="flex-1 truncate">{catName(r.categoryId)} · {r.description || fundName(r.fundId)}</span>
          ...
          <CommandGroup heading="รายจ่าย">
            {results.expense.map((r) => (
              <CommandItem key={r.id} onSelect={() => go("/expense")}>
                ...
                <span className="flex-1 truncate">{catName(r.categoryId)} · {r.vendor || r.description}</span>
                <span className="text-xs text-muted-foreground ml-2">{fmtDate(r.date)}</span>
          ...
          <CommandGroup heading="เงินถวาย">
            {results.offering.map((r) => (
              <CommandItem key={r.id} onSelect={() => go("/offering")}>
                ...
                <span className="flex-1 truncate">{OFFERING_LABEL[r.type]} · {fundName(r.fundId)}</span>
                <span className="text-xs text-muted-foreground ml-2">{fmtDate(r.date)}</span>
          ...
          <CommandGroup heading="งบประมาณ">
            {results.budgets.map((r) => (
              <CommandItem key={r.id} onSelect={() => go("/budget")}>
                ...
                <span className="flex-1 truncate">{r.name}</span>
                <span className="text-xs text-muted-foreground ml-2 tabular-nums">{thb(r.amount)}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        )}
          ...
          <CommandGroup heading="Audit Logs">
            {results.audit.map((r) => (
              <CommandItem key={r.id} onSelect={() => go("/audit")}>
                ...
                <span className="flex-1 truncate">{r.userName} · {r.action} · {r.entity}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        )}
