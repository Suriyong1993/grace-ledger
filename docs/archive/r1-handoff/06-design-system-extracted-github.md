...
- Corrected the Screen map below to the real files that implement each area.
- Labeled `components/*.jsx` and `ui_kits/grace-ledger/` as prototyping-only, not production source.
- Tokens (`colors.css`, `radius.css`, `shadows.css`, `motion.css`) updated for D1 (radius/shadow
  canonicalization) and the `--on-*-muted` relocation — see the repo's `DECISIONS.md`.
...
- `router.ts` uses hash routes: `/`, `/transactions`, `/funds`, `/members`, `/reports`, `/approvals[/:id]`,
  `/offerings[/new|/:id]`. There is no `src/routes/` directory in this repo (a prior version of this document
  implied one).
